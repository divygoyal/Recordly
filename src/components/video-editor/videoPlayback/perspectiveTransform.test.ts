...
describe("compute3DTransform", () => {
  ...
  it("matches expected values at (0.75, 0.25)", () => {
    ...
    // RX = -30 + 0.25*5 = -28.75°, RY = -(20 + 0.75*(-40)) = 10°, RZ = (0.25)*2*0.25*(-4) = -0.5°
  ...
  it("defaults to 30° fov when not specified (FocuSee normal preset)", () => {
    const { fov } = compute3DTransform(
      makeConfig(0.5),
      { cx: 0.5, cy: 0.5 },
      1,
